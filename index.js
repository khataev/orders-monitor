...
const single_working_bot = settings.get("features.single_working_bot");
...
function getToday() {
  if (single_working_bot && single_working_bot != "today") return;
...
function getTomorrow() {
  if (single_working_bot && single_working_bot != "tomorrow") return;
...
function startUpdatesPolling(settings) {
  const update_interval = settings.get("orders.update_interval") * 1000;
  const update_interval_modifier = single_working_bot ? 0 : 1;
  const half_interval = (update_interval / 2) * update_interval_modifier;
